...
class Sound(object):
    ...
    def playFX(self, soundfile):
        '''
        Plays sound effect.
        '''
        self.play_sound("effects", soundfile)
    ...
    def play_sound(self, emitter, soundfile):
        ...
        if not ".ogg" in soundfile:
            soundfile = "sounds/%s.ogg" % soundfile
        emitter.reset()
        #TODO remove str() -- http://github.com/fifengine/fifengine/issues/449
        emitter.setSoundClip(self.soundclipmanager.get(soundfile))
        emitter.play()
